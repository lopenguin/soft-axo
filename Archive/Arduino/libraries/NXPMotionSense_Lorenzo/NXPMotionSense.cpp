...
{
	const uint8_t i2c_addr=FXOS8700_ADDR;
	uint8_t b;
	...
	if (!write_reg(i2c_addr, FXOS8700_CTRL_REG1, 0x15)) return false; // 100Hz A+M (changed from 0x15 to 0x0D to increase to 200  Hz)
...
{
        const uint8_t i2c_addr=FXAS21002_ADDR;
        uint8_t b;
	...
	if (!write_reg(i2c_addr, FXAS21002_CTRL_REG1, 0x0E)) return false; // modified from 0x0E to change to 200 Hz (0x0A)
