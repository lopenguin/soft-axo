...
static constexpr int zeroAngle{100};

bool bangBangAtPushoff(Axo axo, unsigned long timeSinceLastHeelStrike, unsigned long lastStepTime) {
    ...
    if (timeSinceLastHeelStrike > (0.2 + triggerPercent*lastStepTime)) {
        axo.stopMotors();
    }
}
