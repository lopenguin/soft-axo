...
ESC motor1(ESC_PIN, PULSEWIDTH_OFFTIME, CALIBRATION_DELAY, MIDDLE_POINT); // create ESC object to control the motor
ESC motor2(ESC_PIN, PULSEWIDTH_OFFTIME, CALIBRATION_DELAY, MIDDLE_POINT); // create ESC object to control the motor
...
void loop() {
  ...
  if (pulsewidth > 0) {
    ...
    while (millis() - timer < MOTOR_TURN_TIME) {
      motor1.turn(pulsewidth + MIDDLEPOINT); // this line must be repeated in some sort of loop (in order to repeat the motor duty cycle) 
      motor2.turn(-1 * pulsewidth + MIDDLEPOINT);
    }
    Serial.println("");
  }
}
